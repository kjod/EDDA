...
The data is clearly not normally distributed which will filter out any statistical test that is based on this assumption. In this case a permuation test will be used, however to do this the data needs to be formatted in such away in which it has two groups of experimental units.
...
Data is not normally distribute, therefore cannot use anova. A non-parametric test will then be performed, specifically the Kruskal-Wallis test.

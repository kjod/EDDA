...
summary(lm(oxidant~wind+temperature+humidity, data=airpollution))$r.squared
summary(lm(oxidant~wind+temperature+insolation, data=airpollution))$r.squared
summary(lm(oxidant~wind+temperature+day, data=airpollution))$r.squared
summary(lm(oxidant~wind+temperature+insolation+humidity, data=airpollution))$r.squared
summary(lm(oxidant~wind+temperature+insolation+humidity+day, data=airpollution))$r.squared
```

Looking at the $R^2$, it increases when adding more variables, although insignificantly in the last 3 additions. Investigating the summary and the p-value per variable, we can see that insolation, humidity and day do not apport much information.

From the 3 least relevant variables (insolation, humidity and day), humidity seems to be the most relevant one, reaching a p-value of 0.131 (using hypothesis $H_0:\beta_i=0$) when having a wind+temperature+humidity model. Besides this, the p-value is above 0.05 and the increase in $R^2$ is still not significant. Therefore, we do not add it to the model, finishing with a simple oxidant~wind+temperature model.
